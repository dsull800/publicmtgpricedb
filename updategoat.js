...
client.query("SELECT DISTINCT(name) FROM public.cards WHERE rarity IN('mythic','rare') AND setcode IN(SELECT code FROM public.sets WHERE type IN ('expansion','core') and releasedate>=(SELECT releasedate FROM public.sets WHERE mcmname='Return to Ravnica'))",
(err, res) => {
var index=0;
  ...
  setTimeout(function() {   //  call a 3s setTimeout when the loop is called
     ...
     nightmare
          ...
          .then(response=>{
              ...
              client.query('SELECT MAX(carddate) FROM public.totalgoatbot WHERE cardname=$1 AND cardtype=$2',[cardname,cardtypecount],(err1, res1) => {
              ...
              // parsedresponse[1][0].reverse().forEach(function(item){
                ...
                var d1 = new Date(carddate);
                var d2 = new Date(res1['rows'][0]['max']);
